...
$req_referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
$request_headers = getallheaders();
$request_body = file_get_contents('php://input');
//通过request_query字符串生成queries键值对
foreach(explode('&', $request_query) as $value) {  
    $request_query_tmp = explode('=', $value, 2); // 使用第二个参数限制结果为两个元素  
    if (count($request_query_tmp) == 2) {  
        $queries[$request_query_tmp[0]] = $request_query_tmp[1];  
    } else {  
        // 处理错误或忽略无效的查询字符串部分  
        // 例如，可以记录日志或设置默认值  
        $queries[$request_query_tmp[0]] = ''; // 假设没有值，或者根据需要处理  
    }  
}
/*tool*/
